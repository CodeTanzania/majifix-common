...
exports.toE164 = function (phoneNumber, countryCode) {
  ...
  try {
    const DEFAULT_COUNTRY_CODE = getString('DEFAULT_COUNTRY_CODE');
    const _countryCode = (countryCode || DEFAULT_COUNTRY_CODE);
    let _phoneNumber = phone(phoneNumber, _countryCode);
    _phoneNumber = _.first(_phoneNumber).replace(/\+/g, '');
    return _phoneNumber;
  }
